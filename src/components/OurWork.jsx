...
const OurWork = () => {
    ...
    return (
        <motion.div
            ...
            viewport={{ once: true }}
            id='our-work' className=' flex flex-col items-center gap-7 text-white px-4 sm:px-12 lg:px-24 xl:px-40 pt-30 text-gray-700'>
            ...
            <div className='grid sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full max-w-5xl'>
                {
                    workData.map((work, index) => (
                        <motion.div
                            ...
                            <h3 className='mt-3 text-white mb-2 text-lg font-semibold'>{work.title}</h3>
                            <p className='text-sm opacity-60
                            text-white w-5/6'>{work.description}</p>
                        </motion.div>
                    ))
                }
            </div>
